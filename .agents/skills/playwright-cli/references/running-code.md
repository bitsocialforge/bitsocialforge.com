...
For timing-sensitive actions, perform the action and readiness check within one `run-code` invocation so time between CLI calls does not dominate the result.
