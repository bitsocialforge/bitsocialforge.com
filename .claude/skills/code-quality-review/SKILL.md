...
- Do not re-run or duplicate deterministic checks such as the repo verification hook (`scripts/agent-hooks/verify.sh`: local asset references and HTML structure). Mention them only when the diff suggests they are especially relevant.
