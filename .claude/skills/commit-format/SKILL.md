...
6. Always include a scope — a short, human-readable area name (for example `hero`, `master plan`, `agent hooks`), matching how this repo actually commits (see the `commit` skill)
7. Use `perf` for performance optimizations (not `fix`)
8. Description: 2-3 sentences about the solution, no bullet points, only if title isn't enough

## Wrong vs Right

❌ WRONG — missing backticks around title:
```
> **Commit title:** refactor(agent hooks): wire Claude settings
```

❌ WRONG — backticks around individual words instead of whole title:
```
> **Commit title:** refactor(agent hooks): wire `.claude/settings.json`
```

❌ WRONG — missing scope:
```
> **Commit title:** `refactor: wire Claude settings`
```
