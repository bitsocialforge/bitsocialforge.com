if (import.meta.env.DEV && typeof window !== "undefined") {
  ...
  // Keep annotation controls out of automated screenshots.
  if (!flags.__VISUAL_TESTING__ && !flags.__NO_DEV_TOOLBAR__) {
    ...
    Promise.all([import("agentation"), import("react"), import("react-dom/client")])
      .then(([{ Agentation }, { createElement }, { createRoot }]) => {
        if (disposed) return;
        // A separate root leaves the application markup intact.
