...
2. **Group changes into logical commits**
   ...
   - Modified `styles.css` (hero layout fix)
   - Modified `docs/agent-playbooks/deployment.md` (docs update)
...
- **Title format:** Conventional Commits with a **required scope**. The scope should be a short, human-readable name for the area of the codebase affected.
  ...
  | Pattern                    | Example                                |
  | -------------------------- | -------------------------------------- |
  | `type(scope): description` | `feat(master plan): add phases legend` |

- **Never omit the scope.** `feat: add phases legend` is wrong. `feat(master plan): add phases legend` is correct.
