...
```bash
gh pr status
gh pr list --repo bitsocialforge/bitsocialforge.com --state open
gh pr view <pr-number> --repo bitsocialforge/bitsocialforge.com --json number,title,url,headRefName,baseRefName,isDraft,reviewDecision,mergeStateStatus
```
...
gh api "repos/bitsocialforge/bitsocialforge.com/issues/<pr-number>/comments?per_page=100"
gh api "repos/bitsocialforge/bitsocialforge.com/pulls/<pr-number>/reviews?per_page=100"
gh api "repos/bitsocialforge/bitsocialforge.com/pulls/<pr-number>/comments?per_page=100"
```
...
```bash
gh pr comment <pr-number> --repo bitsocialforge/bitsocialforge.com --body "Addressed the valid review findings in the latest commit. Remaining comments were triaged as stale, low-value, or follow-up work that does not block this merge."
```
