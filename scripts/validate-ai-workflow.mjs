...
// Vendored skills that intentionally differ by harness because their command
// syntax, helper agents, and frontmatter are generated per toolchain. Keep this
// allowlist narrow; repo-authored skills should stay mirrored.
const SKILL_CONTENT_EXEMPT_PREFIXES = new Map([
  ['skills/impeccable/', 'Vendored design skill has harness-specific command syntax and helper packaging'],
]);

// Skill support files that intentionally exist in one toolchain only.
const SINGLE_TOOLCHAIN_SKILL_FILES = new Map([
  [
    '.codex/skills/impeccable/agents/impeccable_asset_producer.toml',
    'Codex-only helper agent packaged by the vendored impeccable skill',
  ],
  [
    '.codex/skills/impeccable/agents/impeccable_manual_edit_applier.toml',
    'Codex-only helper agent packaged by the vendored impeccable skill',
  ],
  [
    '.codex/skills/impeccable/agents/openai.yaml',
    'Codex-only helper agent config packaged by the vendored impeccable skill',
  ],
]);
...
function isSkillContentExempt(skillRel) {
  if (CODEX_BODY_EXEMPT.has(skillRel)) return true;
  for (const prefix of SKILL_CONTENT_EXEMPT_PREFIXES.keys()) {
    if (skillRel.startsWith(prefix)) return true;
  }
  return false;
}
  ...
  for (const file of allFiles) {
    ...
    for (const tc of presentIn) {
      if (!holders.includes(tc)) {
        const onlyCopy = holders.length === 1 ? `${holders[0]}/${skillRel}` : null;
        if (onlyCopy && SINGLE_TOOLCHAIN_SKILL_FILES.has(onlyCopy)) continue;
    ...
    for (const tc of holders) {
      ...
      if (isSkillContentExempt(skillRel)) continue;
