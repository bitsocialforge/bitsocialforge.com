...
is_generated_or_lock_file() {
  case "$1" in
    yarn.lock|package-lock.json|pnpm-lock.yaml|bun.lockb|dist/*|build/*|coverage/*|node_modules/*|*.map) return 0 ;;
    *) return 1 ;;
  esac
}
...
is_reviewable_file() {
  case "$1" in
    *.html|*.js|*.jsx|*.ts|*.tsx|*.mjs|*.cjs|*.css|*.json|*.md|*.yml|*.yaml|*.toml|*.sh|Dockerfile) return 0 ;;
    *) return 1 ;;
  esac
}
